...
print '#!/bin/bash'
for nspecies in [32,64,128,256,512,1024,2048]:
	for genespersp in [1,3,5]:
		for mean_len in [0.2]:
			for ind in range(10):
				...
				print 'qsub -cwd -b y -l "hostname=fischer0'+str(random.randint(1,4))+'" ./runCalc.sh '+gtname+" "+stname
				print "sleep 2"
				#continue
...
for nspecies in [2,3,4,8]:
	for genespersp in [20,50,100,200,500,1000]:
		for mean_len in [0.2]:
			for ind in range(10):
				#if justcommandscalc:
				gtname_stells='GT.sp'+str(nspecies)+'.gps'+str(genespersp)+'.len'+str(mean_len)+'ind'+str(ind)+'.nonums'
